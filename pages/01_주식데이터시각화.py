...
def get_series(ticker: str) -> pd.Series:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        ...
        close = df["Close"]
    # 결측 전후 보간 → Prophet NaN 방지
    return close.rename(ticker).ffill().bfill()     # ★ fix
...
use_macro = st.checkbox("거시 변수 포함 (S&P 500·금리·WTI)", value=True)
...
if use_macro:
    for mname, mtic in MACRO_TICKERS.items():
        base = get_series(mtic).reindex(prices.index).ffill().bfill()
        last_val = base.iloc[-1]
        future_vals = pd.concat([
            base,
            pd.Series([last_val] * horizon,
